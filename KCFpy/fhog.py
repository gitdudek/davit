...
def getFeatureMaps(image, k, mapp):
	...
	mapp['sizeY'] = sizeY
	mapp['numFeatures'] = p
	size = math.floor(float(mapp['sizeX']*mapp['sizeY']*mapp['numFeatures']))
	mapp['map'] = np.zeros(size, np.float32)
	...
	'''print(dx.dtype)
	print(dy.dtype)
	print(type(boundary_x))
	print(type(boundary_y))
	print(type(r))
	print(type(alfa))
	print(type(nearest))
	print(type(w))
...
def normalizeAndTruncate(mapp, alfa):
	...
	mapp['sizeX'] = sizeX
	mapp['sizeY'] = sizeY
	mapp['map'] = newData.astype('float32')
