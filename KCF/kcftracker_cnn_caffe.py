...
class KCFTracker:
	...
	def getFeatures(self, image, inithann, scale_adjust=1.0):
		...
		if self.ftype == 'cnn':
			FeaturesMaps =self.feat(z)
			# FeaturesMaps = self.feat(image)
			# temp_img = image
			# temp_img[:,:,0] = FeaturesMaps
			# print("fest1",FeaturesMaps.shape,image.shape,temp_img.shape)
			# FeaturesMap = subwindow(temp_img, extracted_roi, cv2.BORDER_REPLICATE)[:,:,0]
			# FeaturesMap= cv2.resize(FeaturesMap, tuple(map(int,self._tmpl_sz)))
			# print("fest",FeaturesMap.shape)
			FeaturesMap = FeaturesMaps[12,:,:]
			FeaturesMap =cv2.resize(FeaturesMap,(z.shape[1],z.shape[0]))
			cv2.imwrite("vid2/orig%d.jpg" %(self.i),z)
			for x in range(0,64):
			 	feat_img1 = FeaturesMaps[x,:,:]
				feat_img =cv2.resize(feat_img1,(z.shape[1],z.shape[0]))
			 	cv2.imwrite("vid2/orfeat%d_%d.jpg" %(self.i,x),feat_img)
			cv2.imwrite("vid2/feat%d.jpg" %(self.i),FeaturesMap)
			print(FeaturesMap.shape,z.shape)
			self.i+=1 
